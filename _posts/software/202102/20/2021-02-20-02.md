...
## 介绍
`swagger 3.0` `hyperf` 自动生成脚本，方便开发一键生成 `swagger` 文档结构，支持数据库填充返回数据库格式 
> 实例：php bin/hyperf.php swagger:format Post '{"errcode":0,"errmsg":"success","data":{"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9"}}' table1,table2,table3
...
Usage:
  swagger:format [options] [--] [<method> [<response> [<table>]]]

Arguments:
  method                           请求方式 [default: "Get"]
  response                         响应 json 数据类型
  table                            自动填充数据表

Options:
  -P, --path[=PATH]                url路径
  -T, --tag[=TAG]                  标签
  -S, --summary[=SUMMARY]          概要
  -D, --description[=DESCRIPTION]  描述
  -Q, --query[=QUERY]              query格式数据
  -R, --request[=REQUEST]          请求 json 数据类型
...
Help:
  示例：php bin/hyperf.php swagger:format Post '{"errcode":0,"errmsg":"success","data":{"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9"}}'
```
...
class SwaggerCommand extends HyperfCommand
{
    ...
    {
        parent::configure();
        $this->setHelp('示例：php bin/hyperf.php swagger:format Post \'{"errcode":0,"errmsg":"success","data":{"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9"}}\'');
...
eof;
     ...
     * 获取请求 JsonContent 数据.
     *
     * @param string $requestBody 请求json数据
     */
...
eof;
     ...
     * 构造 JsonContent 数据.
     *
     * @param string｜array $data json格式字符串数据｜数组数据
